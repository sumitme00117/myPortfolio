        ...
        <div className="adminPanelYoutubeVideos">
            ...
            {user && user.timeline && user.timeline.map((item)=>(
                <div className='youtubeCard' key={item._id}>
                    ...
                    <Typography variant='body1' style={{letterSpacing: "2px"}}>{item.date.toString().split("T")[0]}</Typography>
                    <Button style={{margin: "auto", display: "block", color: "rgba(40,40,40,0.7)"}} onClick={()=> deleteHandler(item._id)}><FaTrash/></Button>
                </div>
            ))}
        </div>
        </div>
